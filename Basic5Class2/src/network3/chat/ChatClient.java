...
class ChatClient implements ActionListener, Runnable {//인터페이스는 다중으로 상속가능
	JFrame f;
	...
	void changeNameProc(){
		...
		try {
			...
			ta.append("이름변경실패: " + e.toString());
			e.printStackTrace();
		}
		JOptionPane.showMessageDialog(null, nickname + "대화명이 변경되었습니다.");
	}
	...
	void connProc() {
		...
		try {
			s = new Socket(ip, 1234);//소캣생성
			out = s.getOutputStream();//서버 출력
			in = new BufferedReader( new InputStreamReader(s.getInputStream()));//BufferedReader는 문자형 통로, InputStream은 바이트형 이 둘을 호환시켜주는게 InputStreamReaderdlek
			new Thread(this).start();//서버에 접속한 순간부터 run()이 진행될 수 있게
		} catch (Exception e) {
